...
def dispEvent(ID):
    ...
    for m in cursor.fetchall():
        counter+=1
    if counter==0:
        l5=Label(t,text="This Sponsor has not sponsored any event till now!",bg="black",foreground="snow",font="Baskerville 12 bold").grid(row=rowCount,column=i)
        rowCount+=1
    else:
        ...
        rowCount+=1
        f1=("SELECT NAME,TOPIC,YEAR,TYPE FROM EVENTS WHERE E_ID=(SELECT E_ID FROM SPONSOR_EVENT WHERE S_ID=?)")
